...
### BEMUSE / NiHu
Research-oriented BEM libraries (acoustics / engineering BEM stacks).
...
Spectral/hp element CFD (Nektar++); high-order CG CFD–DEM (Lethe); many CFD codes also build on deal.II / MFEM.
...
EDF industrial CFD (Code_Saturne); FE/FV fluids (Fluidity); spectral-element CFD (Nek5000 / nekRS).
...
DEM for granular materials and industrial particle processes (often LAMMPS-related or dedicated DEM stacks).
...
### Code_Aster / CalculiX / Elmer (vibroacoustics)
Structural–acoustic coupling via industrial FEM suites (see FEM section); Elmer and Code_Aster have dedicated acoustic / coupled modules.

- https://code-aster.org/
- https://www.elmerfem.org/

### Bempp / OpenBEM (acoustic BEM)
Exterior and interior Helmholtz acoustics via boundary elements (see BEM section).

- https://bempp.com/
- https://openbem.dk/

### K-Wave / AcouSTO
MATLAB/C++ toolbox for time-domain acoustic / ultrasound wavefields (k-Wave); research acoustic solvers (AcouSTO).
...
Built-in reactingEulerFoam / chemFoam / combustion packages and community combustion forks on the OpenFOAM toolboxes.
...
### ChemApp / CoolProp (thermo adjuncts)
Note: prefer fully open stacks (Cantera, CoolProp) for kinetics/properties in OSS workflows.
...
### Eigen / Armadillo / BLAS-LAPACK stacks
Dense linear algebra building blocks used inside many solvers (plus vendor BLAS such as OpenBLAS).
...
- https://github.com/fffaraz/awesome-cpp — C++ resources (many solver stacks)
